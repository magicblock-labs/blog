...
The V0.1 of the BOLT engine is live. Bolt is a high performance and composable game engine that simplifies the development of games that are permissionless, composable and unstoppable. The design of BOLT includes an SVM-compatible acceleration layer that elevates the performance of fully onchain games on par with traditional multiplayer game server, without compromising the composability of Solana’s global state. In this early release we are going through the BOLT CLI and Entity Component System, addressing the recents changes and explaining how to set up a simple onchain game on Solana. ✨

## The ECS pattern

While Bolt isn't exclusively an Entity Component System (ECS) framework, we encourage the use of this powerful design pattern to enhance composability. ECS is a way of organizing code and data which enables games that are modular and easier to extend, one main benefit of fully onchain games. As the name suggests, in an ECS there are:  
...
This separation of concerns enables a highly flexible and modular architecture. You can explore all the benefits of the ECS pattern here [here](https://github.com/SanderMertens/ecs-faq?tab=readme-ov-file#what-is-ecs. ).

## BOLT CLI

The BOLT CLI is an extension of the Anchor framework. It includes all the features of the most popular Solana development framework, plus a superset of functionalities such as creating world instances, components and systems.  
...
The **#[component]** macro is taking care of all the underlying Solana boilerplate. You don’t have to understand how Solana accounts work, how to allocate bytes or anything else other than defining the data structure.

Components are themselves programs deployed onchain. 

The id defines the unique address of the Position component above.
...
Systems contain the logic that manipulates components. A system will typically operate on all entities that have a specific set of components. The "Movement" system example encapsulates the logic for updating the "Position" component.
...
Now that we have a grasp of how components and systems operate, let’s create a game instance with the World Program from the TypeScript SDK. The World Program is the entrypoint for creating world instances, entities, attaching components, and executing systems, while the SDK provides a convenient interface and methods for interacting with BOLT.
...
Someone would want to introduce a new powerup for faster movement. They can do it by simply defining a new logic that act on the Position component using the new Velocity
...
We can’t wait to see the new mechanics that will emerge out of this pattern. 
